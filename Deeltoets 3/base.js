		...
		swipeUpRefresh: function() {
			window.onscroll = function() {
				...
				if (window.scrollY == 0) {
					...
					Hammer(body).on("dragdown", function(event) {
						event.preventDefault();
						var upAmount = event.gesture.deltaY * 0.8;
						if (upAmount > 150) {
							upAmount = 150;
						}
		       			anim(body, {marginTop: upAmount}, 0.1).anim(function(){
		       				Hammer(body).on("release", function(event) {
									anim(body, {marginTop: 0}, 0.6, "ease-out").anim(function(){
										if (upAmount == 150) {
											FRISBEEAPP.router.init();
										}
										upAmount = 0;
			...
			Hammer(body).on("dragdown", function(event) {
				...
				var upAmount = event.gesture.deltaY * 0.4;
				if (upAmount > 100) {
					upAmount = 100;
				}
       			anim(body, {marginTop: upAmount}, 0.1).anim(function(){
